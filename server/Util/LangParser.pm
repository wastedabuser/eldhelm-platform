...
sub deparse {
	my ($self) = @_;
	$self->{stream} = $self->deparseChunk($self->{syntax}, 0);
...
sub _deparse_array {
	...
	my $i    = 0;
	foreach (@list) {
		$ret .= $self->indent($level + 1).$self->deparseChunk($_, $level + 1);
		$ret .= ",$self->{le}" if $_->[0] ne "comment" && $i < @list - 1;
		$i++;
	}
...
sub _deparse_pair {
	my ($self, $data, $level) = @_;
	return qq~"$data->[1]": ~.$self->deparseChunk($data->[2], $level);
}
